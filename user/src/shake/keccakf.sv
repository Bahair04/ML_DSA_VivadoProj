module keccakf(
    ...
    input   logic   [2 : 0]         din_y,      // 写入位置 y (行)
    input   logic   [2 : 0]         din_x,      // 写入位置 x (列)
...
// --- 中间计算变量 ---
logic   [4 : 0][63 : 0]             bc;                     // Theta 步骤的临时列异或值
logic   [4 : 0][4 : 0][63 : 0]      st_after_theta;         // Theta 后的状态
logic   [4 : 0][4 : 0][63 : 0]      st_after_rho_pi;        // Rho/Pi 后的状态
logic   [4 : 0][4 : 0][63 : 0]      st_after_chi;           // Chi 后的状态
...
always_comb begin
    for (int x = 0 ; x < 5 ; x++) begin
        for (int y = 0 ; y < 5 ; y++) begin
            ...
            rot = get_rotc(x, y);
            if (rot == 0)
                st_after_rho_pi[new_x][new_y] = st[x][y];
            else
                st_after_rho_pi[new_x][new_y] = (st[x][y] << rot) | (st[x][y] >> (64 - rot));
        end
    end
end
...
always_comb begin
    for (int x = 0 ; x < 5 ; x++) begin
        for (int y = 0 ; y < 5 ; y++) begin
            ...
            st_after_chi[x][y] = st[x][y] ^ ((~st[xp1][y]) & st[xp2][y]);
        end
    end
end
