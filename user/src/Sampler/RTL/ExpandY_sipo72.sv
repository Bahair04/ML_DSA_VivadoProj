...
always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
        ...
        // 3. 处理 Consume (输出数据)
        // 检查补充后的长度是否够输出
        if (next_len >= 72) begin
            sipo_o       <= next_reg[71:0]; // 截取低 72 位
            sipo_o_valid <= 1'b1;
            
            // 移位并更新长度
            // 注意：这里是对 next_reg 操作，实现了“同周期既读又写”
            shift_reg     <= next_reg >> 72;
            shift_reg_len <= next_len - 72;
        end
        else begin
            // 不够输出，仅更新补充后的值
            shift_reg     <= next_reg;
...
ExpandA_sipo u_ExpandA_sipo (
  ...
  .rd_en(rd_en & (shift_reg_len < 72)),         // input wire rd_en
